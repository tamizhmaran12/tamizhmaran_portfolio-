...
const DefaultCursorSVG = ({ size = 12, color = "currentColor", className }: { size?: number; color?: string; className?: string }) => {
  return (
    <svg
      ...
      {showTrail &&
        ...
        trail.map((pos, index) => (
          <motion.div
            ...
            className="w-1.5 h-1.5 bg-primary/70 rounded-full pointer-events-none"
          />
        ))}
